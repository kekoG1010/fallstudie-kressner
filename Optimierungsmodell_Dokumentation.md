...
**Bedeutung:** Der Speicher kann zu jedem Zeitpunkt entweder laden ODER entladen, aber nicht beides gleichzeitig. ($M$ = Big-M, z.B. 10.000)
...
$$SOC_{v,t} = SOC_{v,t-1} + \sum_{l \in \mathcal{L}} p^{ch}_{v,l,t} \cdot \Delta t - \sum_{r \in \mathcal{R}(t)} \sum_{f \in \mathcal{F}^E} \phi_{v,r,f} \cdot \kappa^{step}_{r,f}$$

wobei $\kappa^{step}_{r,f} = \frac{\kappa_f}{100} \cdot \frac{d_r}{t^{end}_r - t^{start}_r}$ der Verbrauch pro Zeitschritt ist.

**Aussprache:** „SOC-v-t gleich SOC-v-t-minus-eins plus Laden minus Verbrauch"

**Bedeutung:** Der Ladezustand eines Fahrzeugs ergibt sich aus dem vorherigen SOC plus Ladung minus Energieverbrauch während der Fahrt.
...
$$SOC_{v,t} \leq M \cdot \epsilon_v \quad \forall v \in \mathcal{V}, t \in \mathcal{T}$$
...
$$p^{ch}_{v,l,t} \leq P^{max,Fzg}_{max} \cdot \epsilon_v \quad \forall v \in \mathcal{V}, l \in \mathcal{L}, t \in \mathcal{T}$$
...
**Aussprache:** „Ladeleistung kleiner gleich Säulenleistung mal Belegung und kleiner gleich Fahrzeug-Maximum"

**Bedeutung:** Die Ladeleistung ist begrenzt durch die Säulenleistung, die Belegung des Ladepunkts und die maximale Ladeleistung des Fahrzeugs.
...
$$\omega_{v,t} \geq x_{v,r} \quad \forall v \in \mathcal{V}, t \in \mathcal{T}, r \in \mathcal{R}(t)$$

$$\omega_{v,t} \leq \sum_{r \in \mathcal{R}(t)} x_{v,r} \quad \forall v \in \mathcal{V}, t \in \mathcal{T}$$

**Aussprache:** „Omega-v-t ist eins genau dann wenn Fahrzeug v zum Zeitpunkt t auf einer Route ist"

**Bedeutung:** Die Variable $\omega_{v,t}$ zeigt an, ob ein Fahrzeug gerade unterwegs ist.
...
$$\chi_{v,t} - \chi_{v,t+1} \leq \omega_{v,t+1} \quad \forall v \in \mathcal{V}, t \in \mathcal{T}^{Nacht}$$

**Aussprache:** „Chi-v-t minus Chi-v-t-plus-eins kleiner gleich Omega-v-t-plus-eins"

**Bedeutung:** Ein laufender Ladevorgang darf nicht unterbrochen werden, außer das Fahrzeug fährt auf Route. Unterbrechung zum „Zwischenladen" eines anderen LKW ist verboten.
...
| Komponente | Anzahl |
|------------|--------|
| Binärvariablen | ca. 15.000 |
| Kontinuierliche Variablen | ca. 5.000 |
| **Variablen gesamt** | **ca. 20.000** |
| **Nebenbedingungen** | **ca. 62.500** |
...
*Dokumentation erstellt für DHBW Fallstudie SCM – Elektrifizierung der Logistik*
